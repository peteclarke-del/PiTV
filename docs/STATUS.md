...
| P5 | The copy rule makes more transcoding than a Pi can do. One day's manifest for seven channels is 397 copies (101 hours, 104 GB) and 107 transcodes (90 hours of programme). 66 of those are H.264 or HEVC files transcoded only for being taller than 864 lines, which the Pi 4 decodes in hardware; 35 are standard definition MPEG-4 it decodes in software; 6 are VP9 or AV1. Proposed: copy whatever the Pi can play and transcode only the rest. It changes requirement 51's neighbour in the contract (section 2) and costs cache space and a live downscale, so it is Pete's decision | Decision recorded; the manifest's transcode count for a day is one a Pi can finish overnight |
